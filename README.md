...
* **Analytics Choice**: Analytics is on by default for new installs and upgrades. Turn off **Share usage and reliability data** under Settings → Advanced at any time; the opt-out survives updates. Turning analytics off attempts one final opt-out event, then stops tracking. The installation ID stays on your device while analytics is off. Re-enabling reuses it to connect future activity with earlier activity; nothing is collected or replayed from the opted-out period. Clear All Data preserves both your installation ID and analytics preference. Events include setup/feature activity, sync counts and durations, fixed error categories, extension version, and a random installation identifier. See [analytics configuration and event definitions](docs/analytics.md).
* **Secure Storage**: Your API tokens are encrypted before being stored locally on your machine.
* **Data Removal**: Uninstalling removes local storage. **Clear All Data** removes local credentials, cached data, configuration, and analytics session/activity state, retaining your installation ID and current analytics preference. Neither operation deletes analytics already received by Google.
